...
export default function Home() {
  ...
  const projects = [
    {
      ...
      description: "BFS, DFS, Dijkstra ve A* algoritmalarını gerçek zamanlı olarak görselleştiren interaktif bir araç. Duvar çizebilir, çamur engeli ekleyebilir ve algoritmaların nasıl çalıştığını adım adım izleyebilirsin.",
      details: [
        "BFS, DFS, Dijkstra ve A* algoritmaları",
        "Gerçek zamanlı görselleştirme",
        "Duvar & ağırlıklı hücre desteği",
        "Labirent otomatik oluşturma",
        "Çalışma süresi & maliyet analizi"
      ],
      github: "https://github.com/Cyhnern/AI-Pathfinding-Visualizer",
      demo: "https://cyhnern.github.io/AI-Pathfinding-Visualizer/"
    },
    {
      title: "🖥️ MIPS Pipeline Simulator",
      description: "5 aşamalı MIPS pipeline simülatörü. Data Hazard, Load-Use Hazard ve Control Hazard tespiti yaparak CPI ve performans analizi sunar.",
      details: [
        "5 aşamalı pipeline görselleştirmesi (IF, ID, EX, MEM, WB)",
        "Data Hazard (RAW) tespiti",
        "Load-Use Hazard tespiti (1 stall)",
        "Control Hazard tespiti",
        "CPI ve performans analizi"
      ],
      github: "https://github.com/Cyhnern/MIPS-Pipeline-Simulator",
      demo: "https://mips-pipeline-simulator.onrender.com/"
    },
    {
      title: "🤖 ML Model Visualizer",
      description: "Overfitting, Regularization, Dropout ve eğitim grafiklerini interaktif olarak görselleştiren makine öğrenmesi aracı. Gerçek scikit-learn modelleri ile çalışır.",
      details: [
        "Overfitting vs Underfitting görselleştirme",
        "L1/L2 Regularization karşılaştırma",
        "Gerçek zamanlı eğitim grafikleri (50 epoch)",
        "Dropout & Ensemble etkisi gösterimi",
        "Plotly.js ile interaktif grafikler"
      ],
      github: "https://github.com/Cyhnern/ML-Model-Visualizer",
      demo: "https://ml-model-visualizer.onrender.com/"
    }
  ];
  ...
  useEffect(() => {
    if (loadingStep < loaderSteps.length) {
      const timer = setTimeout(() => setLoadingStep(prev => prev + 1), 700);
    ...
    const handleScroll = () => {
      ...
      for (const section of sections) {
        ...
        if (el) {
          ...
          if (rect.top <= 160 && rect.bottom >= 160) { setActiveSection(section); break; }
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const type = () => {
      ...
      if (isDeleting) { setTypewriterText(currentWord.substring(0, charIndex - 1)); charIndex--; }
      else { setTypewriterText(currentWord.substring(0, charIndex + 1)); charIndex++; }
      let typeSpeed = isDeleting ? 40 : 80;
      if (!isDeleting && charIndex === currentWord.length) { typeSpeed = 1500; isDeleting = true; }
      else if (isDeleting && charIndex === 0) { isDeleting = false; wordIndex = (wordIndex + 1) % typewriterWords.length; typeSpeed = 400; }
  ...
  const handleTerminalSubmit = (e: React.FormEvent) => {
    ...
    if (cmd === "help") response.push("Kullanılabilir komutlar: whoami, skills, projects, contact, clear");
    else if (cmd === "whoami") response.push("Ceyhun Eren - Bilgisayar Mühendisliği Öğrencisi & Yapay Zeka Meraklısı.");
    else if (cmd === "skills") response.push("Python/Yapay Zeka: %95", "Web Geliştirme (Next.js/Tailwind): %90", "Java & C (MIPS Mimari): %85", "SQL Veri Tabanı: %80");
    else if (cmd === "projects") response.push("4 Proje Listelendi:", "- AI Pathfinding Visualizer", "- MIPS Pipeline CPU Simulator", "- Portfolio V2 (Şu an buradasınız)", "- Data Analysis Dashboard");
    else if (cmd === "contact") response.push("Instagram: @cyhnern", "GitHub: github.com/Cyhnern", "E-posta: cyhnern9@gmail.com");
    else if (cmd === "clear") { setTerminalHistory([]); setTerminalInput(""); return; }
    else response.push(`'${cmd}' komutu bulunamadı. Yardım almak için 'help' yazın.`);
  ...
  const handleLogoClick = () => {
    ...
    if (clickCount + 1 === 5) { alert("👀 Sırrı Keşfettin! Geliştirici Moduna Hoş Geldin Ceyhun. Başarılar dilerim! 🚀"); setClickCount(0); }
  };
        ...
        <div className="w-full max-w-lg bg-[#111827] rounded-xl p-6 border border-slate-800 shadow-2xl">
          ...
          <div className="space-y-2 text-sm text-slate-300 min-h-[100px]">
            {loaderSteps.slice(0, loadingStep).map((step, idx) => (
              <div key={idx} className={idx === loaderSteps.length - 1 ? "text-green-400 font-bold" : ""}>{step}</div>
            ))}
          </div>
          <div className="mt-6">
            <div className="text-xs text-slate-500 mb-1 flex justify-between"><span>Loading Portfolio...</span><span>{Math.min(loadingStep * 25, 100)}%</span></div>
            <div className="w-full h-1.5 bg-slate-800 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#13294B] to-[#FED141] transition-all duration-500" style={{ width: `${loadingStep * 25}%` }} />
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <main className={`min-h-screen font-sans relative transition-colors duration-500 overflow-hidden ${darkMode ? 'bg-slate-900 text-slate-100' : 'bg-white text-[#13294B]'}`}>
      ...
      {selectedProject && (
        ...
        >
          <div
            className={`relative w-full max-w-lg rounded-3xl p-8 shadow-2xl border ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-100'}`}
            onClick={e => e.stopPropagation()}
          >
            <button onClick={() => setSelectedProject(null)} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-300 transition-all font-bold text-sm">✕</button>
            <h3 className="text-2xl font-black mb-3">{selectedProject.title}</h3>
            <p className={`text-sm font-medium mb-5 leading-relaxed ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{selectedProject.description}</p>
            <ul className="space-y-2 mb-6">
              {selectedProject.details.map((d, i) => (
                <li key={i} className="flex items-center gap-2 text-sm font-semibold">
                  <span className="text-[#FED141] font-black">✓</span> {d}
                </li>
              ))}
            </ul>
            <div className="flex gap-3">
              <a href={selectedProject.demo} target="_blank" rel="noopener noreferrer" className="flex-1 text-center px-4 py-3 rounded-xl font-black text-sm bg-[#13294B] text-white hover:bg-[#13294B]/80 transition-all">🚀 Demoyu Aç</a>
              <a href={selectedProject.github} target="_blank" rel="noopener noreferrer" className={`flex-1 text-center px-4 py-3 rounded-xl font-black text-sm border transition-all ${darkMode ? 'bg-slate-800 border-slate-600 hover:bg-slate-700' : 'bg-slate-100 border-slate-200 hover:bg-slate-200'}`}>GitHub</a>
            </div>
          </div>
        </div>
      )}
      ...
      {showAllProjects && (
        ...
        >
          <div
            className={`relative w-full max-w-2xl rounded-3xl p-8 shadow-2xl border ${darkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-slate-100'}`}
            onClick={e => e.stopPropagation()}
          >
            <button onClick={() => setShowAllProjects(false)} className="absolute top-4 right-4 w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-300 transition-all font-bold text-sm">✕</button>
            <h3 className="text-2xl font-black mb-6">🧠 Yapay Zeka & Mühendislik Projeleri</h3>
            <div className="space-y-4">
              {projects.map((project, i) => (
                <div
                  key={i}
                  className={`p-5 rounded-2xl border cursor-pointer transition-all hover:scale-[1.01] hover:border-[#FED141]/50 ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-200'}`}
                  onClick={() => { setShowAllProjects(false); setSelectedProject(project); }}
                >
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-black text-lg">{project.title}</h4>
                    <span className={`text-xs font-bold px-2 py-1 rounded-lg ${darkMode ? 'bg-slate-700 text-[#FED141]' : 'bg-[#13294B]/5 text-[#13294B]'}`}>Detay →</span>
                  </div>
                  <p className={`text-sm font-medium ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{project.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="pointer-events-none fixed inset-0 z-30 transition-opacity duration-300 opacity-40 dark:opacity-20 hidden md:block"
        style={{ background: `radial-gradient(600px circle at ${mousePos.x}px ${mousePos.y}px, ${darkMode ? 'rgba(254,209,65,0.07)' : 'rgba(19,41,75,0.05)'}, transparent 80%)` }} />
        ...
        <div className="flex justify-between items-center max-w-6xl mx-auto px-6 py-4 font-bold">
          ...
          <div className="flex items-center gap-6 text-sm">
            {[
              ...
              <a key={item.id} href={`#${item.id}`} className={`transition-all flex items-center gap-1 ${activeSection === item.id ? 'text-[#FED141] font-black' : 'hover:opacity-70'}`}>
                {activeSection === item.id && <span className="text-xs">►</span>}
                {item.label}
              </a>
            ))}
            <button onClick={() => setDarkMode(!darkMode)} className={`p-2 rounded-xl border transition-all ${darkMode ? 'bg-slate-800 border-slate-700 text-[#FED141]' : 'bg-slate-100 border-slate-200 text-[#13294B]'}`}>
              {darkMode ? (
        ...
        <div className="flex justify-center relative min-h-[400px] items-end group">
          <div className={`w-72 h-[420px] rounded-[2.5rem] rotate-2 shadow-2xl border-4 flex flex-col items-center justify-between p-4 hover:rotate-0 transition-all duration-300 hover:scale-105 cursor-pointer relative z-10 overflow-hidden ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-[#13294B] border-white'}`}>
            ...
            <div className="w-full h-[300px] rounded-2xl overflow-hidden">
              <img src="/profil.png" alt="Ceyhun Eren" className="w-full h-full object-cover" />
            </div>
      ...
      <section className={`py-12 border-y ${darkMode ? 'bg-slate-800/20 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center font-bold">
          <div><div className="text-4xl font-black text-[#FED141]">6+</div><div className="text-xs uppercase tracking-wider opacity-60 mt-1">Tamamlanan Proje</div></div>
          <div><div className="text-4xl font-black text-[#FED141]">45+</div><div className="text-xs uppercase tracking-wider opacity-60 mt-1">GitHub Commit</div></div>
          <div><div className="text-4xl font-black text-[#FED141]">10+</div><div className="text-xs uppercase tracking-wider opacity-60 mt-1">Blog Yazısı</div></div>
          <div><div className="text-4xl font-black text-[#FED141]">4</div><div className="text-xs uppercase tracking-wider opacity-60 mt-1">Programlama Dili</div></div>
        </div>
      </section>
          ...
          {skills.map((skill, index) => (
            <div key={index} className={`p-6 rounded-2xl border transition-all hover:scale-[1.02] shadow-sm ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
              ...
              <div className={`w-full h-3 rounded-full overflow-hidden ${darkMode ? 'bg-slate-700' : 'bg-slate-100'}`}>
                <div className="h-full rounded-full bg-gradient-to-r from-[#13294B] to-[#FED141] transition-all duration-1000 ease-out" style={{ width: `${skill.level}%` }} />
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="ilgi-alanlari" className={`py-24 px-6 transition-colors duration-500 relative z-10 ${darkMode ? 'bg-slate-900' : 'bg-[#FED141]/10'}`}>
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
            {/* Kart 1: Yapay Zeka — tıklayınca tüm projeler listesi açılır */}
            <div
              onClick={() => setShowAllProjects(true)}
              className={`border rounded-3xl p-8 shadow-md transition-all duration-300 transform hover:-translate-y-3 hover:shadow-2xl hover:border-[#FED141]/50 group relative overflow-hidden cursor-pointer ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}
            >
              ...
              <div className={`inline-flex items-center gap-1 text-xs font-black px-3 py-1.5 rounded-lg ${darkMode ? 'bg-slate-700 text-[#FED141]' : 'bg-[#13294B]/5 text-[#13294B]'}`}>
                Projeleri Gör ({projects.length}) →
              </div>
            </div>
        ...
        <div className="w-full bg-[#0b0f19] rounded-2xl border border-slate-800 shadow-2xl overflow-hidden font-mono text-sm text-slate-200">
          ...
          <form onSubmit={handleTerminalSubmit} className="bg-[#111827] px-6 py-3 flex items-center gap-2 border-t border-slate-800">
            ...
            <input type="text" value={terminalInput} onChange={(e) => setTerminalInput(e.target.value)}
              ...
              className="bg-transparent border-none outline-none flex-1 text-white focus:ring-0 p-0" autoComplete="off" />
          </form>
        </div>
      </section>
      ...
      <section id="iletisim" className="max-w-4xl mx-auto px-6 py-24 text-center relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-6">
          <a href="https://instagram.com/cyhnern" target="_blank" rel="noopener noreferrer" className={`px-5 py-3 rounded-xl font-bold border transition-all transform hover:-translate-y-1 ${darkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'}`}>Instagram</a>
          <a href="https://github.com/Cyhnern" target="_blank" rel="noopener noreferrer" className={`px-5 py-3 rounded-xl font-bold border transition-all transform hover:-translate-y-1 ${darkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'}`}>GitHub</a>
          <a href="mailto:cyhnern9@gmail.com" className={`px-5 py-3 rounded-xl font-bold border transition-all transform hover:-translate-y-1 ${darkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-200 hover:bg-slate-50 shadow-sm'}`}>E-posta</a>
        </div>
      </section>
